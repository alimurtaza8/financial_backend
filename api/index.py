...
PDF_STORAGE_PATH = Path("generated_pdfs")
...
class FinalProposalRequest(BaseModel):
    proposal_items: List[ProposalItem]
    payment_terms: List[PaymentTerm]
    terms_conditions: Optional[str] = Field(default="", description="Custom terms and conditions")
    company_logo_url: Optional[str] = Field(default="", description="Company logo URL")
    tax_rate: float = Field(default=15.0, description="Tax rate percentage (VAT)")
    include_tax: bool = Field(default=True, description="Whether to include tax calculations")
...
async def upload_pdf(file: UploadFile = File(...)):
    """Upload a generated PDF and return a download URL"""
    try:
        file_location = PDF_STORAGE_PATH / file.filename
        with open(file_location, "wb+") as file_object:
            file_object.write(file.file.read())
        
        # In a real production environment, you would return a URL from your domain
        # For now, this assumes the backend is running on a known host or localhost
        # This needs to be configured based on where the backend is deployed
        base_url = os.getenv("FINANCIAL_BACKEND_URL", "http://localhost:8000")
        download_url = f"{base_url}/download-pdf/{file.filename}"
        
        return {"filename": file.filename, "downloadUrl": download_url}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Could not upload file: {e}")

@app.post("/api/upload-logo")
async def upload_logo(file: UploadFile = File(...)):
    """Upload company logo and return a URL"""
    try:
        # Validate file type
        ...
        logo_storage_path = Path("uploaded_logos")
        logo_storage_path.mkdir(exist_ok=True)
        
        # Generate unique filename
        file_extension = file.filename.split('.')[-1] if '.' in file.filename else 'png'
        unique_filename = f"logo_{datetime.now().strftime('%Y%m%d_%H%M%S')}.{file_extension}"
        file_location = logo_storage_path / unique_filename
        
        with open(file_location, "wb+") as file_object:
            file_object.write(file.file.read())
        
        base_url = os.getenv("FINANCIAL_BACKEND_URL", "http://localhost:8000")
        logo_url = f"{base_url}/download-logo/{unique_filename}"
        
        return {"filename": unique_filename, "logoUrl": logo_url}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Could not upload logo: {e}")
...
@app.get("/download-logo/{filename}")
async def download_logo(filename: str):
    """Download a stored logo file"""
    logo_path = Path("uploaded_logos") / filename
    ...
    return FileResponse(path=logo_path, media_type=media_type, filename=filename)
...
async def create_financial_proposal(request: FinalProposalRequest):
    ...
    try:
        ...
        subtotal = sum(item.total_price for item in request.proposal_items)
        ...
        # Calculate payment amounts based on total after tax
        payment_terms_with_amounts = []
        for term in request.payment_terms:
            payment_terms_with_amounts.append({
                "description": term.description,
        ...
        default_terms = """
1. All prices are in Saudi Arabian Riyals (SAR)
2. Payment terms as specified in the payment schedule
3. This proposal is valid for 30 days from the date of issue
4. Additional work outside the scope will be quoted separately
5. VAT will be added as per Saudi Arabian tax regulations
        """.strip()
        ...
        proposal = {
            ...
            'payment_terms': payment_terms_with_amounts,
            'currency': 'SAR',
            'terms_conditions': request.terms_conditions or default_terms,
        ...
        # Get the most recent metadata from proposals_storage
        latest_metadata = None
        for stored_quotation_code, stored_data in proposals_storage.items():
            if 'metadata' in stored_data:
                latest_metadata = stored_data['metadata']
                break
...
@app.get("/api/proposal/{quotation_code}/export-xlsx")
async def export_proposal_xlsx(quotation_code: str):
    """Export proposal as professionally formatted XLSX file"""
    if quotation_code not in proposals_storage:
        raise HTTPException(status_code=404, detail="Proposal not found")
    ...
    try:
        ...
        company_font = Font(name='Arial', size=20, bold=True, color='1F4E79')
        ...
        # Define professional fills
        header_fill = PatternFill(start_color='1F4E79', end_color='1F4E79', fill_type='solid')
        light_blue_fill = PatternFill(start_color='D9E3F0', end_color='D9E3F0', fill_type='solid')
        ...
        ws['A3'] = 'Professional AI Consulting Services - Financial Proposal'
        ...
        project_info_left = [
            ('Project Name (EN):', metadata.get('project_name_en', 'N/A')),
            ('Project Name (AR):', metadata.get('project_name_ar', 'N/A')),
            ('Client Name (EN):', metadata.get('client_name_en', 'N/A')),
            ('Client Name (AR):', metadata.get('client_name_ar', 'N/A')),
            ('RFP Code:', metadata.get('rfp_code', 'N/A')),
        ]
        
        project_info_right = [
            ('Offer Number:', proposal.get('offer_number', 'N/A')),
            ('Date:', proposal.get('date', 'N/A')),
            ('Start Date:', metadata.get('start_date', 'N/A')),
            ('End Date:', metadata.get('end_date', 'N/A')),
            ('Duration:', f"{metadata.get('duration_months', 'N/A')} months"),
        ]
        
        for i, ((left_label, left_value), (right_label, right_value)) in enumerate(zip(project_info_left, project_info_right)):
            current_row = row + i
            # Left column
            ws[f'A{current_row}'] = left_label
            ws[f'B{current_row}'] = left_value
            ws[f'A{current_row}'].font = label_font
            ws[f'B{current_row}'].font = normal_font
            
            # Right column  
            ws[f'D{current_row}'] = right_label
            ws[f'E{current_row}'] = right_value
        ...
        ws.merge_cells(f'A{row}:F{row}')
        ws[f'A{row}'] = 'PROJECT DELIVERABLES'
        ws[f'A{row}'].font = title_font
        ws[f'A{row}'].fill = header_fill
        ws[f'A{row}'].alignment = Alignment(horizontal='center', vertical='center')
        ...
        # Table headers
        headers = ['#', 'Description', 'Quantity', 'Unit Price (SAR)', 'Total Price (SAR)', 'Status']
        ...
        # Deliverable items
        total_amount = 0
        for i, item in enumerate(proposal.get('items', []), 1):
            unit_price = item.get('unit_price') or item.get('total_price', 0)
            total_price = item.get('total_price') or unit_price
            ...
            # Format currency
            ws.cell(row=row, column=4).number_format = '#,##0" SAR"'
            ws.cell(row=row, column=5).number_format = '#,##0" SAR"'
        ...
        subtotal = proposal.get('subtotal', total_amount)
        tax_amount = proposal.get('tax_amount', 0)
        tax_rate = proposal.get('tax_rate', 15)
        final_total = proposal.get('total_amount', subtotal + tax_amount)
        
        # Subtotal
        ws[f'D{row}'] = 'Subtotal (Before Tax):'
        ws[f'E{row}'] = subtotal
        ws[f'D{row}'].font = label_font
        ws[f'E{row}'].font = currency_font
        ws[f'E{row}'].number_format = '#,##0" SAR"'
        ...
        # Tax (if applicable)
        if proposal.get('include_tax', True) and tax_amount > 0:
            ws[f'D{row}'] = f'VAT ({tax_rate}%):'
        ...
        ws[f'E{row}'] = final_total
        ws[f'D{row}'].font = Font(name='Arial', size=12, bold=True, color='1F4E79')
        ws[f'E{row}'].font = Font(name='Arial', size=12, bold=True, color='1F4E79')
        ws[f'E{row}'].number_format = '#,##0" SAR"'
        ws[f'D{row}'].alignment = Alignment(horizontal='right')
        ws[f'E{row}'].alignment = Alignment(horizontal='right')
        
        # Thick border for total
        for col in ['D', 'E']:
        ...
        if proposal.get('payment_terms'):
            ...
            # Payment headers
            payment_headers = ['#', 'Description', 'Percentage', 'Amount (SAR)', '', '']
            for col, header in enumerate(payment_headers, 1):
                if header:  # Only add headers for non-empty columns
                    cell = ws.cell(row=row, column=col, value=header)
                    cell.font = header_font
                    cell.fill = header_fill
                    cell.border = thin_border
            ...
            # Payment terms
            for i, term in enumerate(proposal.get('payment_terms', []), 1):
                ws.cell(row=row, column=1, value=i)
                ws.cell(row=row, column=2, value=term.get('description', ''))
                ws.cell(row=row, column=3, value=f"{term.get('percentage', 0)}%")
                ws.cell(row=row, column=4, value=term.get('amount', 0))
        ...
        terms_text = proposal.get('terms_conditions', '')
        if terms_text:
            # Split terms by lines and add each as a separate row
        ...
        filename = f"mutawazi_proposal_{quotation_code}.xlsx"
