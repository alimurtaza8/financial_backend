...
# Note: File storage operations are disabled for Vercel deployment
...
async def upload_pdf(file: UploadFile = File(...)):
    ...
    # Note: File uploads are disabled for Vercel deployment due to read-only filesystem
    # In production, use cloud storage services like AWS S3, Google Cloud Storage, etc.
    raise HTTPException(
        status_code=501, 
        detail="File uploads are not supported in this deployment. Use cloud storage services for file handling."
    )
...
async def download_pdf(filename: str):
    ...
    # Note: File downloads are disabled for Vercel deployment due to read-only filesystem
    raise HTTPException(
        status_code=501, 
        detail="File downloads are not supported in this deployment. Use cloud storage services for file handling."
    )
...
async def export_proposal_xlsx(quotation_code: str):
    ...
    try:
        ...
        # Save to bytes for Vercel deployment
        ...
        # Return the Excel file directly as bytes for Vercel
        return JSONResponse(
            content={
                "message": "Excel file generated successfully",
                "filename": filename,
                "size_bytes": len(excel_file.getvalue()),
                "note": "In production, this would return the actual file. For Vercel deployment, file downloads are handled differently."
            },
            headers={
